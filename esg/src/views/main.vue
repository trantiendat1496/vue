    ...
    <v-app-bar color="white" app clipped-left elevation="1">
      <v-container fluid class="pa-0 fill-height white--text flex-nowrap mx-n4 mx-sm-0">
        <v-app-bar-nav-icon
          ...
          class="justify-sm-start d-lg-none"
        ></v-app-bar-nav-icon>
          ...
          <v-card width="360px" class="black--text px-3 py-3">
            <v-list-item class="d-flex align-center mb-3" link>
              ...
              <div class="d-inline-flex flex-column">
                <span class="text-h5 font-weight-normal text-no-wrap text-truncate"
                  >責任投資調査部</span
                >
                <span class="primary--text text-caption font-weight-medium"
            ...
            <v-list class="my-0">
              <v-list-item
                class="px-1"
                v-for="({ title, link }, i) in menuItems"
                :key="i"
                link
                :to="link"
              >
            ...
            <v-divider></v-divider>
            <v-list-item
              class="d-flex align-center mt-3 px-1 py-0"
              link
              v-on:click="logOut()"
            >
...
export default class MainContainer extends Vue {
  ...
  // onSelectedItem(item: { brandCd: any }) {
